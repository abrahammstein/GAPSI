...
class GAPSIAlertVC: UIViewController {

    private let containerView   = GAPSIAlertContainerView()
    private let titleLabel      = GAPSITitleLabel(textAlignment: .center, fontSize: 20)
    private let messageLabel    = GAPSIBodyLabel(textAlignment: .center)
    private let actionButton    = GAPSIButton(backgroundColor: .systemPink, title: "Ok")
    ...
    private func configureMessageLabel() {
        messageLabel.text           = message ?? "Unable to complete search request."
